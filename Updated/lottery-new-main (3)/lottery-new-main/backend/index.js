...
app.post("/uservalidate", function (req, res) {
  ...
  let sql =
    "select id,txtFname,refUserRole ,txtUemail from tblusers where txtUemail='" +
    username +
    "' and txtUpassword='" +
    password +
    "';";
...
app.post("/insertuser", (req, res) => {
  ...
  let sql =
    "insert into tblusers (txtFname,txtLname,txtUpassword,txtUemail,refUserRole) values('" +
    fname +
    "','" +
    lname +
    "','" +
...
app.post("/insertunitnumber", (req, res) => {
  ...
  var sql =
    "insert into tblunit(refUser,refLotterymaster,txtLotteryNumber,purchasedetails) values ";
  let first = true;
  for (const obj of arr) {
    if (obj.length > 0) {
      if (first) {
...
app.post("/latestdrawunits", (req, res) => {
  var sql =
    "SELECT l.txtLotteryname, l.dtLotterydrawdate,  l.txtLotteryresult FROM lotterydrums.tbllotterymaster l where   l.txtLotteryresult is not null;";
  con.query(sql, function (err, result) {
    if (err) res.send(err);
    //  console.log(result);
    res.send(result);
...
app.post("/admindash_fetchall", (req, res) => {
  let sql =
    "SELECT lm.txtLotteryname,tp.txtProvidername,COUNT(DISTINCT (CASE WHEN tusr.refUserRole = 2 THEN tu.id END)) 'unitsold',CEIL(((COUNT(DISTINCT (CASE WHEN tusr.refUserRole = 2 THEN tu.id END)) * (lm.txtPurchasedamount - lm.txtCost)) / lm.txtCost)) AS extraticket,(COUNT(DISTINCT (CASE WHEN tusr.refUserRole = 2 THEN tu.id END)) + CEIL(((COUNT(DISTINCT (CASE WHEN tusr.refUserRole = 2 THEN tu.id END)) * (lm.txtPurchasedamount - lm.txtCost)) / lm.txtCost))) AS unitstopurchase,COUNT(DISTINCT (CASE WHEN tu.purchasedetails = 1 THEN tu.id END)) 'purchased',((COUNT(DISTINCT (CASE WHEN tusr.refUserRole = 2 THEN tu.id END)) + CEIL(((COUNT(DISTINCT (CASE WHEN tusr.refUserRole = 2 THEN tu.id END)) * (lm.txtPurchasedamount - lm.txtCost)) / lm.txtCost))) - (COUNT(DISTINCT (CASE WHEN tu.purchasedetails = 1 THEN tu.id END)))) AS balance FROM tblunit tu LEFT JOIN tblusers tusr ON tu.refUser = tusr.id LEFT JOIN tbllotterymaster lm ON tu.refLotterymaster = lm.id LEFT JOIN tblprovider tp ON tp.id = lm.refProvider GROUP BY refLotterymaster;";
  con.query(sql, (err, result) => {
    if (err) res.send(err);
    res.send(result);
...
app.post("/usermail", (req, res) => {
  ...
  //   console.log("val",val.txtLotteryname)
  // }
  ...
  html +=
    "<p style='color:red; display: flex;justify-content: center;width:50%'>LotteryDrums</p>";
  html +=
    "<div style='background-color: lightgrey;width:50%; padding-left: 10px;padding-top: 5px;padding-bottom: 10px ;'><label style='font-weight: bold;'>NUMBERS SUCCESSFULLY SELECTED</label>";
  html += "</div>";
  html += "<div>";
  html +=
    " <p>Dear User,</p><p  style='padding-top: 20px;'>Your have Successfully seleceted your numbers for Kerala Lottery draw.</p></div>";
  // html+="<div style='display: flex;flex-direction: row;width:60%'>"
  // html+="<label style='width:20%'>Raffle ID</label>"
  // html+="<label style='width:20%'>Date</label>"
  // html+="<label style='width:20%'>Selected Numbers</label></div>"
  // html+="<div style='display: flex;width:60%'>"
  html +=
    "<table><tr><th>Raffle Id</th><th>Lotterty Name</th><th>Selected Numbers</th></tr>";
  for (const val of array) {
    html +=
      " <tr><th>'" +
      val.id +
  ...
  var mailOptions = {
    from: "archanacs154@gmail.com",
    to: email,
    subject: "LotteryDrum purchase confirmation",
    html: html,
  };
...
/*************************************************************************PURCHASE LIST OF TICKETS ADMINPAGE******************************************************************************************** */
app.post("/ticketpurchaselist", (req, res) => {
  let sql =
    "select tu.id,tu.txtLotteryNumber,tm.txtLotteryname,tm.dtLotterydrawdate from tblunit tu join tbllotterymaster tm on tu.refLotterymaster=tm.id where tm.dtLotterydrawdate>now() && tu.purchasedetails=0 order by tm.dtLotterydrawdate;";
  con.query(sql, (err, result) => {
    if (err) throw err;
    res.send(result);
    console.log(result);
...
app.post("/updatepurchase", (req, res) => {
  let id = req.body.id;
  let sql = "update tblunit set purchasedetails=1 where id='" + id + "';";
  con.query(sql, (err, result) => {
    if (err) throw err;
    res.send(result);
  });
